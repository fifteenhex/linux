...
static int __init mvme147_scc_request_irqs(struct mvme147_scc_serial_port* sccp_a,
...
fail_free_b_rx:
	free_irq(MVME147_IRQ_SCCB_RX, sccp_b);
fail_free_b_stat:
	free_irq(MVME147_IRQ_SCCB_STAT, sccp_b);
fail_free_b_tx:
	free_irq(MVME147_IRQ_SCCB_TX, sccp_b);
fail_free_a_spcond:
	free_irq(MVME147_IRQ_SCCA_SPCOND, sccp_a);
fail_free_a_rx:
	free_irq(MVME147_IRQ_SCCA_RX, sccp_a);
fail_free_a_stat:
	free_irq(MVME147_IRQ_SCCA_STAT, sccp_a);
fail_free_a_tx:
	free_irq(MVME147_IRQ_SCCA_TX, sccp_a);
...
static int __init mvme147_scc_init(void)
{
...

	{
		...
		printk("%s:%d - %d\n", __func__, __LINE__, MVME147_IRQ_SCC_BASE);
	...
	error = mvme147_scc_request_irqs(&ports[0], &ports[1]);
	if (error)
		return error;
...
static int __init mvme147_scc_serial_init(void)
{
	struct device *dev;
	int res = -ENODEV;
	int i;
	
	/* Create a platform device to root to */
	pdev = platform_device_register_simple("MVME147_SCC", -1, NULL, 0);
	if (!pdev)
		return -ENOMEM;
...
module_init(mvme147_scc_serial_init);
module_exit(mvme147_scc_serial_exit);
