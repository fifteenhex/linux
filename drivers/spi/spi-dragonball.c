...
static irqreturn_t dragonball_spi_irq(int irq, void *dev_id)
{
	...
	cont = __raw_readw(spi->base + REG_CONT);
	__raw_writew((cont & ~REG_CONT_IRQ), spi->base + REG_CONT);
	complete(&spi->done);
			...
			struct spi_device *device, struct spi_transfer *t)
{
	...
	u16 cont = __raw_readw(spi->base + REG_CONT);
	/*
	...
	for (i = 0; i < t->len; i += 2) {
		...
		__raw_writew(out, spi->base + REG_DATA);
		__raw_writew((cont | REG_CONT_XCH), spi->base + REG_CONT);
		...
		if (t->rx_buf) {
			u16 in = __raw_readw(spi->base + REG_DATA);
			if (dotwo) {
				inb[1] = in & 0xff;
				in >>= 8;
			}
...
static int dragonball_spi_probe(struct platform_device *pdev)
{
	...
	cont &= ~REG_CONT_IRQEN;
	__raw_writew(cont, spi->base + REG_CONT);
